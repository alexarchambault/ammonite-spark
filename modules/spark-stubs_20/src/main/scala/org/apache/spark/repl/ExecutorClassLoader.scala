      ...
      if (connection.getResponseCode != 200) {
        ...
        try
          ...
          case ioe: IOException =>
            log.error("Exception while closing error stream", ioe)
        }
  ...
  def findClassLocally(name: String): Option[Class[_]] = {
    ...
    try {
      ...
      case e: ClassNotFoundException =>
        ...
        log.debug(s"Did not load class $name from REPL class server at $uri", e)
        None
      case e: Exception =>
        // Something bad happened while checking if the class exists
        log.error(s"Failed to check existence of class $name on REPL class server at $uri", e)
        None
    }
    finally
      if (inputStream != null)
        try
          ...
          case e: Exception =>
            log.error("Exception while closing inputStream", e)
        }
  }
